...
/**
 * This is the rule ThisCallOnStaticMethodToStaticCallRector from Rector but that works in PHPUnit classes
 *
 * @see https://getrector.com/rule-detail/this-call-on-static-method-to-static-call-rector
 */
final class ThisCallOnStaticMethodToStaticCallEvenInPhpUnit extends AbstractScopeAwareRector implements MinPhpVersionInterface
{
    /**
     * @readonly
...
CODE_SAMPLE
    ...
    private function processThisToStatic(Class_ $class, ClassReflection $classReflection) : void
    {
        $this->traverseNodesWithCallable($class, function (Node $subNode) use($class, $classReflection) {
            ...
            if ($methodName === 'assertResponseRedirects') {
                var_dump($methodName, $isStaticMethod);
            }
